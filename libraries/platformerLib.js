...
class spike {
    ...
    show() {
        ...
        if (this.r == 1 || this.r == 3) {
            for (var x1 = this.x; x1 < ((this.w * (player1.w)) + this.x); x1 += (player1.w) + 1) {
                if (this.r == 1) {
                    triangle(x1, this.y, x1 + (player1.w), this.y, x1 + (player1.w*0.5), this.y - (player1.w));
                } else if (this.r == 3) {
                    triangle(x1, this.y, x1 + (player1.w), this.y, x1 + (player1.w*0.5), this.y + (player1.w));
                }
            }
        } else if (this.r == 2 || this.r == 4) {
            for (var y1 = this.y; y1 < ((this.w * (player1.w)) + this.y); y1 += (player1.w) + 1) {
                if (this.r == 2) {
                    triangle(this.x, y1, this.x + (player1.w), y1 + (player1.w*0.5), this.x, y1 + (player1.w));
                } else if (this.r == 4) {
                    triangle(this.x, y1, this.x - (player1.w), y1 + (player1.w*0.5), this.x, y1 + (player1.w));
                }
            }
